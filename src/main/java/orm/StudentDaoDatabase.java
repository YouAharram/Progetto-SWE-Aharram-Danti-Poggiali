...
public class StudentDaoDatabase implements StudentDao {
    ...
    @Override
    public Student getStudentByUsernameAndPassword(String username, String password) throws StudentDaoException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    throw new StudentDaoException("Student with username " + username + " doesn't exist.");
                }
            }
        } catch (SQLException e) {
            throw new StudentDaoException("Database error while fetching student data");
        }
    }
